...
class Configuration:
    ...
    nlp_initial_embedding_from: List[str] = field(
        default_factory=lambda: ["DDP", "ISO"])
    ddp_projection: List[str] = field(
        default_factory=lambda: ["pca", "barvinok"])
...
def main():
    ...
    for i, combination in enumerate(combinations, start=1):
        ...
        combination = combination + \
            (os.path.join(output_path, cmd_path_suffix),)
